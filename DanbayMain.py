...
def generateExcel():
    ...
    row0 = [u'房间地址', u'中控管家密码数', u'中控租客密码数(含预置)', u'中控临时密码个数(含预置)', u'云端租客密码数', u'云端管家密码数', u'门锁已使用密码数', u'门锁密码容量']
...
def generateMeterExcel():
    ...
    row0 = [u'房间地址', u'类型', u'状态', u'30天内离线总时长(单位:天)',u'30天内离线次数', u'当前读数',u'设备Mac', u'设备ID',  u'中控mac', u'中控状态', u'中控device id']
    for i in range(0, len(row0)):
...
def generateShuiDianExcelFromDB():
    f = xlwt.Workbook(encoding='utf-8')  # 创建工作簿
    sheet1 = f.add_sheet(u'水电表信息统计', cell_overwrite_ok=True)  # 创建sheet
    row0 = [u'房间地址', u'类型', u'状态', u'设备Mac', u'设备ID', u'表头读数', u'离线时间', u'在线时间', u'中控状态', u'中控id', u'中控mac', u'中控版本号',
            u'中控关联地址']
    for i in range(0, len(row0)):
...
def generateCaiJiQi():
    f = xlwt.Workbook(encoding='utf-8')  # 创建工作簿
    sheet1 = f.add_sheet(u'水电表信息统计', cell_overwrite_ok=True)  # 创建sheet
    row0 = [u'房间地址', u'类型', u'状态', u'30天内离线总时长(单位:天)', u'30天内离线次数', u'当前读数', u'采集器Mac', u'采集器ID', u'设备表号', u'设备ID',u'中控Mac',u'中控状态',u'中控deviceId'
           ]
    for i in range(0, len(row0)):
        sheet1.col(0).width = 256 * 40
